...
"""Triton kernels for Attention with logit softcapping.

Derived from attention.py. Gemma-2 squashes every pre-softmax score through a
tanh so no single key can dominate:
...
@triton.jit
def softcap_attention_naive_prefill(
    query_base,
    key_base,
    value_base,
    kv_cache_base,
...
@triton.jit
def softcap_attention_naive_decode(
    query_base,
    key_base,
    value_base,
    kv_cache_base,
...
@triton_op("rbln_triton_ops::softcap_attention_naive_prefill", mutates_args=())
def softcap_attention_naive_prefill_wrapper(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    ...
    warmup(softcap_attention_naive_prefill, *params)

    return output.to(original_dtype)


@triton_op("rbln_triton_ops::softcap_attention_naive_decode", mutates_args=())
def softcap_attention_naive_decode_wrapper(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    kv_cache: torch.Tensor,
    ...
    warmup(softcap_attention_naive_decode, *params)

    return output.to(original_dtype)


@register_fake("rbln_triton_ops::softcap_attention_naive_prefill")
def _(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
...
@register_fake("rbln_triton_ops::softcap_attention_naive_decode")
def _(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
