...
"""Triton kernels for Attention with an additive bias.

Derived from attention.py. The mask argument carries an additive bias
instead of a keep-mask, which is how a per-head positional bias such as
ALiBi gets in: the paged pass matches on the operand signature, so a new
tensor argument is not available.
"""
...
@triton.jit
def additive_attention_naive_prefill(
    query_base,
    key_base,
    value_base,
    kv_cache_base,
    ...
    for batch_id in tl.static_range(0, NUM_BATCH, 1):
        ...
        additive_mask_ptr = tl.make_block_ptr(
            base=additive_mask_base,
            shape=(NUM_BATCH, NUM_HEAD, NUM_GROUP, QUERY_LEN, PARTITION_SIZE),
            strides=(
                NUM_HEAD * NUM_GROUP * QUERY_LEN * PARTITION_SIZE,
                NUM_GROUP * QUERY_LEN * PARTITION_SIZE,
                QUERY_LEN * PARTITION_SIZE,
                PARTITION_SIZE,
                1,
            ),
            offsets=(batch_id, 0, 0, 0, 0),
            block_shape=(1, NUM_HEAD, NUM_GROUP, QUERY_LEN, PARTITION_SIZE),
            order=(4, 3, 2, 1, 0),
        )
        ...
        # 2.2 b = a * qk_scale
        additive_mask = tl.load(additive_mask_ptr)  # (1,h,g,l,p)
        qk = qk + additive_mask
        qk_scaled = qk * qk_scale  # (1,h,g,l,p)
        boolean_mask = tl.exp(qk_scaled - qk_scaled)  # all ones
        softmax_masked_qk_scaled = rblib.masked_softmax(qk_scaled, boolean_mask)
...
@triton.jit
def additive_attention_naive_decode(
    query_base,
    key_base,
    value_base,
    kv_cache_base,
...
@triton_op("rbln_triton_ops::additive_attention_naive_prefill", mutates_args=())
def additive_attention_naive_prefill_wrapper(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    ...
    warmup(additive_attention_naive_prefill, *params)

    return output.to(original_dtype)


@triton_op("rbln_triton_ops::additive_attention_naive_decode", mutates_args=())
def additive_attention_naive_decode_wrapper(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    kv_cache: torch.Tensor,
    ...
    warmup(additive_attention_naive_decode, *params)

    return output.to(original_dtype)


@register_fake("rbln_triton_ops::additive_attention_naive_prefill")
def _(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
...
@register_fake("rbln_triton_ops::additive_attention_naive_decode")
def _(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
